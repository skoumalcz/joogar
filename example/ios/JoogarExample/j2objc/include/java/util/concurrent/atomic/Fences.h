...
/*!
 ...
 where declaring variables <code>volatile</code> or <code>final</code>, using
 instances of atomic classes, using <code>synchronized</code> blocks or
 ...
 writes). Method <code>orderWrites</code> is typically used to enforce
 order between two writes, and <code>orderAccesses</code> between a write
 and a read.  Method <code>orderReads</code> is used to enforce order
 between two reads with respect to other <code>orderWrites</code> and/or
 <code>orderAccesses</code> invocations.  The formally specified
 ...
 <p>Each ordering method accepts a <code>ref</code> argument, and
 ...
 the ordering properties associated with <code>volatile</code> and
 ...
 <p><b>Reachability.</b> Method <code>reachabilityFence</code>
 establishes an ordering for strong reachability (as defined in the
 <code>java.lang.ref</code> package specification) with respect to
 garbage collection.  Method <code>reachabilityFence</code> differs from
 ...
 <p><b>Sample Usages</b>
 <p><b>Safe publication.</b> With care, method <code>orderWrites</code>
 may be used to obtain the memory safety effects of <code>final</code>
 for a field that cannot be declared as <code>final</code>, because its
...
@endcode
 Here, the invocation of <code>orderWrites</code> ensures that the
 effects of the widget assignment are ordered before those of any
 (unknown) subsequent stores of <code>h</code> in other variables that
 make <code>h</code> available for use by other objects.  Initialization
 sequences using <code>orderWrites</code> require more care than those
 involving <code>final</code> fields.  When <code>final</code> is not used,
 compilers cannot help you to ensure that the field is set correctly
 across all usages.  You must fully initialize objects
 <em>before</em> the <code>orderWrites</code> invocation that makes
 ...
 to rewrite class <code>WidgetHolder</code> to use <code>final</code>.
...
@endcode
 In this case, the <code>orderWrites</code> invocation occurs before the
 ...
 <code>c</code> contains an accessible variable <code>data</code> that should
 have been declared as <code>volatile</code> but wasn't:
 @code
...
@endcode
 Method <code>getData</code> provides an emulation of <code>volatile</code>
 reads of (non-long/double) fields by ensuring that the read of
 <code>c</code> obtained as an argument is ordered before subsequent
 reads using this reference, and then performs the read of its
 field. Method <code>setData</code> provides an emulation of volatile
 ...
 fence methods would surround calls to methods such as <code>c.getData()</code>
 .  However, these techniques cannot be applied to
 <code>long</code> or <code>double</code> fields because reads and writes of
 ...
 the safety properties of <code>volatile</code>. However their use does
 ...
 for programs using <code>volatile</code>. In particular, emulation using
 Fence methods is not guaranteed to maintain the property that
 <code>volatile</code> operations performed by different threads are
...
@endcode
 Because this construction avoids use of <code>orderAccesses</code>,
 which is typically more costly than the other fence methods, it may
 result in better performance than using <code>volatile</code> or its
 emulation. However, as is the case with most applications of fence
 methods, correctness relies on the usage context -- here, the
 thread safety of <code>Item</code>, as well as the lack of need for full
 volatile semantics inside this class itself. However, the second
 concern means that it can be difficult to extend the <code>ItemHolder</code>
 ...
 in method <code>Object.finalize</code>, which may otherwise run
 concurrently.
 @code
...
@endcode
 Here, the call to <code>reachabilityFence</code> is nonintuitively
 placed <em>after</em> the call to <code>update</code>, to ensure that
 the array slot is not nulled out by <code>Object.finalize</code> before
 the update, even if the call to <code>action</code> was the last use of
 this object. This might be the case if for example a usage in a
 user program had the form <code>new Resource().action();</code> which
 retains no other reference to this Resource.  While probably
 overkill here, <code>reachabilityFence</code> is placed in a <code>finally</code>
  block to ensure that it is invoked across all paths in the
 method.  In a method with more complex control paths, you might
 need further precautions to ensure that <code>reachabilityFence</code>
...
@endcode
 <p>Method <code>reachabilityFence</code> is not required in
 ...
 class Resource (including <code>finalize</code>) were enclosed in <code>synchronized (this)</code>
 ...
 method <code>reachabilityFence</code> remains a better option in cases
 ...
 <li><em>wf</em>, an invocation of <code>orderWrites(p)</code> or
 <code>orderAccesses(p)</code>
 <li><em>w</em>, a write of value <em>p</em>
 <li> <em>rf</em>, an invocation of <code>orderReads(p)</code> or
 <code>orderAccesses(p)</code>
 ...
 <ul>
 <li><em>p</em>, a reference to an object
 <li><em>f</em>, an invocation of <code>orderAccesses(p)</code>
 </ul>
 If:
 <ul>
 <li>sequenced(f, w)
 ...
 <ul>
 <li><em>p</em>, a reference to an object
 <li><em>f</em>, an invocation of <code>reachabilityFence(p)</code>
 <li><em>a</em>, an access where accesses(a, p)
 <li><em>b</em>, an action (by a garbage collector) taking
 the form of an invocation of <code>p.finalize()</code>
  or of enqueing any <code>java.lang.ref.Reference</code>
  constructed with argument <em>p</em>
 </ul>
 If:
 <ul>
 <li>sequenced(a, f)
...
/*!
 @brief Informally: Ensures that accesses (reads or writes) using the
 given reference prior to the invocation of this method occur
 before subsequent accesses.
 For details, see the class
...
/*!
 @brief Informally: Ensures that a read of the given reference prior to
 ...
 invocations of <code>orderWrites</code> and/or <code>orderAccesses</code>
...
/*!
 @brief Informally: Ensures that a use of the given reference with the
...
/*!
 @brief Ensures that the object referenced by the given reference
 remains <em>strongly reachable</em> (as defined in the <code>java.lang.ref</code>
 ...
 Invocation of this method does not
